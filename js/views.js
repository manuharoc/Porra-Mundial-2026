...
function renderRanking(){
  ...
  tbody.innerHTML = scores.map((p,i)=>{
    ...
    return `<tr><td><span class="pos-badge ${bc}">${pos}</span>${trendHtml}</td><td class="name-col"><div style="display:flex;align-items:center;gap:8px">${renderAvatarHtml(p, 'sm')}<span>${p.name}${youMark}${adminMark}</span></div></td><td class="pts-col">${p.grupos}</td><td>${p.r32}</td><td>${p.octavos}</td><td>${p.cuartos}</td><td>${p.semis}</td><td>${p.final}</td><td style="color:var(--accent2)">${p.campeon}</td><td>${p.sub}</td><td style="color:var(--accent)">${p.customPts}</td><td class="pts-col" style="font-size:18px">${p.total}</td></tr>`;
  }).join('');
  ...
  mob.innerHTML = scores.map((p,i)=>{
    ...
    return `<div class="ranking-card ${topClass}">
      ...
      <div class="rc-info">
        ...
        <div class="rc-detail">Grupos: <b>${p.grupos}</b> · Elim: <b>${p.r32+p.octavos+p.cuartos+p.semis+p.final}</b> · Esp: <b>${p.campeon+p.sub+p.customPts}</b>${champStr?' · '+champStr:''}</div>
      </div>
